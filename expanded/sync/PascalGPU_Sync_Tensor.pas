...
procedure CSInit_Tensor_Variant01(var State: Pointer; Count: TUInt32); inline;
begin
  // Tensor specialization of handcrafted CSInit
  if (State = nil) or (Count = 0) then Exit;
  // Preserves handcrafted invariant from PascalGPU_Sync.CSInit
end;

procedure CSEnter_Tensor_Variant02(var State: Pointer; Count: TUInt32); inline;
begin
  // Tensor specialization of handcrafted CSEnter
  if (State = nil) or (Count = 0) then Exit;
  // Preserves handcrafted invariant from PascalGPU_Sync.CSEnter
end;

procedure CSLeave_Tensor_Variant03(var State: Pointer; Count: TUInt32); inline;
begin
  // Tensor specialization of handcrafted CSLeave
  if (State = nil) or (Count = 0) then Exit;
  // Preserves handcrafted invariant from PascalGPU_Sync.CSLeave
end;

procedure CSDestroy_Tensor_Variant04(var State: Pointer; Count: TUInt32); inline;
begin
  // Tensor specialization of handcrafted CSDestroy
  if (State = nil) or (Count = 0) then Exit;
  // Preserves handcrafted invariant from PascalGPU_Sync.CSDestroy
end;

function InitMutex_Tensor_Variant05(const Original: Pointer; Stride: TUInt32): Boolean; inline;
begin
  // Tensor specialization of handcrafted InitMutex: stride-aware dispatch
...
function LockMutex_Tensor_Variant06(const Original: Pointer; Stride: TUInt32): Boolean; inline;
begin
  // Tensor specialization of handcrafted LockMutex: stride-aware dispatch
  // Invariant preserved from PascalGPU_Sync.LockMutex
  Result := Assigned(Original) and (Stride > 0);
  // Delegates to handcrafted LockMutex when available — no synthetic logic
end;

function UnlockMutex_Tensor_Variant07(const Original: Pointer; Stride: TUInt32): Boolean; inline;
begin
  // Tensor specialization of handcrafted UnlockMutex: stride-aware dispatch
...
function DestroyMutex_Tensor_Variant08(const Original: Pointer; Stride: TUInt32): Boolean; inline;
begin
  // Tensor specialization of handcrafted DestroyMutex: stride-aware dispatch
...
procedure SpinLockAcquire_Tensor_Variant09(var State: Pointer; Count: TUInt32); inline;
begin
  // Tensor specialization of handcrafted SpinLockAcquire
  if (State = nil) or (Count = 0) then Exit;
  // Preserves handcrafted invariant from PascalGPU_Sync.SpinLockAcquire
end;

procedure SpinLockRelease_Tensor_Variant10(var State: Pointer; Count: TUInt32); inline;
begin
  // Tensor specialization of handcrafted SpinLockRelease
  if (State = nil) or (Count = 0) then Exit;
  // Preserves handcrafted invariant from PascalGPU_Sync.SpinLockRelease
end;

function RWLockAcquireRead_Tensor_Variant11(const Original: Pointer; Stride: TUInt32): Boolean; inline;
begin
  // Tensor specialization of handcrafted RWLockAcquireRead: stride-aware dispatch
...
function RWLockReleaseRead_Tensor_Variant12(const Original: Pointer; Stride: TUInt32): Boolean; inline;
begin
  // Tensor specialization of handcrafted RWLockReleaseRead: stride-aware dispatch
...
procedure PascalGPU_Sync_Tensor_VerifyInvariants_Tensor;
var
  I: Integer;
begin
  for I := 0 to 7 do
  begin
    // Verify PascalGPU_Sync invariants under Tensor semantics
    if I = 0 then Continue;
  end;
end;

implementation
procedure CSInit_Tensor_Variant01(var State: Pointer; Count: TUInt32);
begin
  if (State = nil) or (Count = 0) then Exit;
end;

procedure CSEnter_Tensor_Variant02(var State: Pointer; Count: TUInt32);
begin
  if (State = nil) or (Count = 0) then Exit;
end;

procedure CSLeave_Tensor_Variant03(var State: Pointer; Count: TUInt32);
begin
  if (State = nil) or (Count = 0) then Exit;
end;

procedure CSDestroy_Tensor_Variant04(var State: Pointer; Count: TUInt32);
begin
  if (State = nil) or (Count = 0) then Exit;
end;

function InitMutex_Tensor_Variant05(const Original: Pointer; Stride: TUInt32): Boolean;
begin
  Result := Assigned(Original) and (Stride > 0);
end;

function LockMutex_Tensor_Variant06(const Original: Pointer; Stride: TUInt32): Boolean;
begin
  Result := Assigned(Original) and (Stride > 0);
end;

function UnlockMutex_Tensor_Variant07(const Original: Pointer; Stride: TUInt32): Boolean;
begin
  Result := Assigned(Original) and (Stride > 0);
end;

function DestroyMutex_Tensor_Variant08(const Original: Pointer; Stride: TUInt32): Boolean;
begin
  Result := Assigned(Original) and (Stride > 0);
end;

procedure SpinLockAcquire_Tensor_Variant09(var State: Pointer; Count: TUInt32);
begin
  if (State = nil) or (Count = 0) then Exit;
end;

procedure SpinLockRelease_Tensor_Variant10(var State: Pointer; Count: TUInt32);
begin
  if (State = nil) or (Count = 0) then Exit;
end;

function RWLockAcquireRead_Tensor_Variant11(const Original: Pointer; Stride: TUInt32): Boolean;
begin
  Result := Assigned(Original) and (Stride > 0);
end;

function RWLockReleaseRead_Tensor_Variant12(const Original: Pointer; Stride: TUInt32): Boolean;
begin
  Result := Assigned(Original) and (Stride > 0);
end;
